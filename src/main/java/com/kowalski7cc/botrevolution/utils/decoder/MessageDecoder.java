...
public class MessageDecoder {
    ...
    public static Message decode(JSONObject object) {
        return new Message(object.getInt("message_id"),
                ...
                .setFrom(Optional.ofNullable(object.optJSONObject("from")).map(UserDecoder::decode).orElse(null))
                .setForwardFrom(Optional.ofNullable(object.optJSONObject("forward_from")).map(UserDecoder::decode).orElse(null))
                .setForwardFromChat(Optional.ofNullable(object.optJSONObject("forward_from_chat")).map(ChatDecoder::decode).orElse(null))
                .setForwardFromMessageID(getInteger(object.optInt("forward_from_message_id")))

                // TODO rewrite as Optional everything
